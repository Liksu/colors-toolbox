...
export type HexColorStop = { color: HexColor, stop: number }
export type InputColorsPalette = Array<HexColor | HexColorStop>
...
export class ColorHelpers {
    ...
    public static inputColorToStop(color: HexColor | HexColorStop): HexColorStop {
        if (typeof (color as HexColor) === 'string') {
            return { color, stop: 0 } as HexColorStop
        }
        
        return color as HexColorStop
    }
    ...
    public static getGradient(colors: InputColorsPalette, length: number, useRGB = false): Gradient {
        ...
        let stops = colors.map(ColorHelpers.inputColorToStop)

        if (stops.length === 1) {
            return ColorHelpers.getOneColorGradient(stops[0].color, length)
        }

        if (stops.length >= length) {
            return stops.slice(0, length).map(s => ColorHelpers.hexToGradientItem(s.color))
        }

        stops = stops.sort((a, b) => a.stop - b.stop)
        if (stops[0].stop !== 0) stops.unshift({ color: stops[0].color, stop: 0 })
        if (stops.at(-1)!.stop !== 1) stops.push({ color: stops.at(-1)!.color, stop: 1 })

        const segments = stops.slice(1).map(({stop}, i) => length * (stop - stops[i].stop))
        
        // const segmentCount = stops.length - 1
        // const segments = Array.from({ length: segmentCount }, () => 2)
        // let sum = stops.length
        // let pointer = 0
        // while (sum < length) {
        //     sum++
        //     segments[pointer]++
        ...
        return segments
            .map((segmentLength, i) =>
                ColorHelpers.getTwoColorsGradient([stops[i], stops[i + 1]], segmentLength, useRGB).slice(i && 1)
            )
            .flat()
    }
}
